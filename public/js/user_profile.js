...
$(document).ready(function() {
    $(".upload-btn").on("click", function () {
        try {
            ...
            doc_parent_div.find(".pic").removeClass('is-invalid');
            let fileInput = doc_parent_div.find(".pic")[0];
            let file_array = fileInput.files[0];
            let username = $("#username").val();
            let type = file_array["type"].split("/");
            let uniqueFileName = `profile-pic-${username.slice(
                1,
                5
            )}-${username.slice(-5)}.${type[1]}`;
            $("#profile_pic").val(uniqueFileName);
            ...
            let file_type = doc_parent_div
                .find(".pic")
                .data("file-type"); // eg. pic/png, image/jpp
    ...
    $(".delete-btn").on("click", function () {
        let this_btn = $(this);
        let doc_parent_div = this_btn.closest(".input-group");
        let fileName = $("#profile_pic").val();
    
        // Call StoragePut with a callback function
        StorageDelete(fileName, function (resp) {
            if (resp) {
                doc_parent_div.find(".pic").val(null);
    ...
    //on form submit console all form inputs
    $("#form").submit(function (e) {
        e.preventDefault();
        // get all form input values and console
        let formData = {};
